...
def summarize_performance(step, g_model, dataset, n_samples=3):
    ...
    if not os.path.exists('models'):
        os.mkdir('models')
    # save plot to file
    filename1 = 'models/plot_%06d.png' % (step + 1)
    ...
    filename2 = 'models/model_final.h5'
    g_model.save(filename2)
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
    ...
    for i in range(n_steps):
        ...
        print(f'Total Epochs={n_epochs}, bat_per_epo={bat_per_epo}, {n_steps}>{i+1},  d1[{d_loss1:.3f}] d2[{d_loss2:.3f}] g[{g_loss:.3f}]')
